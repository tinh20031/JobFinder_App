...
function CustomTabBarButton({ children, onPress, profileCompletion = 0 }) {
  ...
  console.log('ProfileCompletion in CustomTabBarButton:', profileCompletion);
  ...
  const progressColor = profileCompletion >= 40 ? '#4ade80' : '#ef4444'; // Xanh nếu >= 40%, đỏ nếu < 40%
  console.log('ProgressColor:', progressColor, 'for percentage:', profileCompletion);
  
  // Tạo progress ring bằng cách sử dụng nhiều View
  const renderProgressRing = () => {
    if (profileCompletion <= 0) return null;
    
    const segments = [];
    const segmentAngle = 15; // Mỗi segment 15 độ
    const numSegments = Math.ceil(360 / segmentAngle);
    const filledSegments = Math.ceil((progressAngle / 360) * numSegments);
      ...
      segments.push(
        <View
          ...
          style={{
            ...
            borderBottomColor: isFilled ? progressColor : 'transparent',
...
function CustomTabBar({ state, descriptors, navigation }) {
  ...
  console.log('ProfileCompletion in CustomTabBar:', profileCompletion);
  ...
  return (
    ...
    >
      {state.routes.map((route, index) => {
        ...
        if (isProfile) {
          return (
            <CustomTabBarButton key={route.key} onPress={onPress} profileCompletion={profileCompletion}>
              <MaterialIcons
                name={iconName}
                size={28}
                color="#fff"
